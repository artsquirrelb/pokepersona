        ...
        if (!gPaletteFade.active)
        {
            if (gInitialSummaryScreenCallback != NULL)
            {
                ...
                case RELEARN_MODE_BOX_PSS_PAGE_BATTLE_MOVES:
                    if (SWSH_SUMMARY_SCREEN)
                        ShowPokemonSummaryScreen_SwSh(SUMMARY_MODE_RELEARNER_BATTLE, GetBoxedMonPtr(gSpecialVar_MonBoxId, 0), gSpecialVar_MonBoxPos, IN_BOX_COUNT - 1, gInitialSummaryScreenCallback);
                    else
