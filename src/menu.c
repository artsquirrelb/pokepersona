...
void AddTextPrinterForMessage(bool8 allowSkippingDelayWithButtonPress)
{
    ...
    AddTextPrinterParameterized2(0, FONT_NORMAL, gStringVar4, GetPlayerTextSpeedDelay(), NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, 0);//TEXT_COLOR_LIGHT_GRAY);
}

void AddTextPrinterWithCustomSpeedForMessage(bool8 allowSkippingDelayWithButtonPress, u8 speed)
{
    gTextFlags.canABSpeedUpPrint = allowSkippingDelayWithButtonPress;
    AddTextPrinterParameterized2(0, FONT_NORMAL, gStringVar4, speed, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, 0);//TEXT_COLOR_LIGHT_GRAY);
}
...
static void WindowFunc_RedrawDialogueFrame(u8 bg, u8 left, u8 top, u8 width, u8 height, u8 paletteNum)
{
    FillMenuTilemapBufferRect(bg,  0, left - 2,         top - 1,         1, 1);
    FillMenuTilemapBufferRect(bg,  1, left - 1,         top - 1,         1, 1);
    FillMenuTilemapBufferRect(bg,  1, left,             top - 1, width - 1, 1);
...
static void WindowFunc_DrawDialogueFrame(u8 bg, u8 left, u8 top, u8 width, u8 height, u8 paletteNum)
{
    ...
    FillMenuTilemapBufferRect(bg, 12, left + width - 1, top + height,         1, 1);
    FillMenuTilemapBufferRect(bg, 13, left + width,     top + height,         1, 1);
}
...
void AddTextPrinterWithCallbackForMessage(bool8 canSpeedUp, void (*callback)(struct TextPrinterTemplate *, u16))
{
    ...
    AddTextPrinterParameterized2(0, FONT_NORMAL, gStringVar4, GetPlayerTextSpeedDelay(), callback, TEXT_COLOR_DARK_GRAY, 0, 0);
}
...
void RedrawMenuCursor(u8 oldPos, u8 newPos)
{
    ...
    AddTextPrinterParameterizedCustom(sMenu.windowId, sMenu.fontId, gText_SelectorArrow3, sMenu.left, sMenu.optionHeight * newPos + sMenu.top, 0, 0, 0x2, 0x0, 0x0);
}
...
void PrintMenuActionTexts(u8 windowId, u8 fontId, u8 left, u8 top, u8 letterSpacing, u8 lineHeight, u8 itemCount, const struct MenuAction *menuActions, const u8 *actionIds)
{
    ...
    printer.windowId = windowId;
    printer.fontId = fontId;
    printer.color.foreground = 1; //GetFontAttribute(fontId, FONTATTR_COLOR_FOREGROUND);
    printer.color.background = 0;//x3;//GetFontAttribute(fontId, FONTATTR_COLOR_BACKGROUND);
    printer.color.shadow = 0;//GetFontAttribute(fontId, FONTATTR_COLOR_SHADOW);
    printer.color.accent = 0;//GetFontAttribute(fontId, FONTATTR_COLOR_ACCENT);
...
static void CreateYesNoMenuAtPos(const struct WindowTemplate *window, u8 fontId, u8 left, u8 top, u16 baseTileNum, u8 paletteNum, u8 initialCursorPos)
{
    ...
    printer.color.background = 0x3;//GetFontAttribute(fontId, FONTATTR_COLOR_BACKGROUND);
...
void PrintMenuActionGrid(u8 windowId, u8 fontId, u8 left, u8 top, u8 optionWidth, u8 horizontalCount, u8 verticalCount, const struct MenuAction *menuActions, const u8 *actionIds)
{
    ...
    printer.fontId = fontId;
    printer.color.foreground = 1;//GetFontAttribute(fontId, FONTATTR_COLOR_FOREGROUND);
    printer.color.background = 0;//GetFontAttribute(fontId, FONTATTR_COLOR_BACKGROUND);
    printer.color.shadow = 0;//GetFontAttribute(fontId, FONTATTR_COLOR_SHADOW);
    printer.color.accent = 0;//GetFontAttribute(fontId, FONTATTR_COLOR_ACCENT);
    printer.letterSpacing = GetFontAttribute(fontId, FONTATTR_LETTER_SPACING);
    printer.lineSpacing = GetFontAttribute(fontId, FONTATTR_LINE_SPACING);
...
static void MoveMenuGridCursor(u8 oldCursorPos, u8 newCursorPos)
{
    ...
    FillWindowPixelRect(sMenu.windowId, PIXEL_FILL(3), xPos, yPos, cursorWidth, cursorHeight);
    ...
    AddTextPrinterParameterizedCustom(sMenu.windowId, sMenu.fontId, gText_SelectorArrow3, xPos, yPos, 0, 0, 0x2, 0x0, 0x0);
}
...
void PrintMenuTable(u8 windowId, u8 itemCount, const struct MenuAction *menuActions)
{
    ...
    for (i = 0; i < itemCount; i++)
        ...
        AddTextPrinterParameterizedCustom(windowId, FONT_NORMAL, menuActions[i].text, 8, (i * 16) + 1, TEXT_SKIP_DRAW, NULL, 0x1, 0x0, 0x0);
    CopyWindowToVram(windowId, COPYWIN_GFX);
}
...
void PrintMenuActionTextsInUpperLeftCorner(u8 windowId, u8 itemCount, const struct MenuAction *menuActions, const u8 *actionIds)
{
    ...
    printer.color.foreground = 1;//GetFontAttribute(FONT_NORMAL, FONTATTR_COLOR_FOREGROUND);
    printer.color.background = 0;//GetFontAttribute(FONT_NORMAL, FONTATTR_COLOR_BACKGROUND);
    printer.color.shadow = 0;//GetFontAttribute(FONT_NORMAL, FONTATTR_COLOR_SHADOW);
    printer.color.accent = 0;//GetFontAttribute(FONT_NORMAL, FONTATTR_COLOR_ACCENT);
    printer.letterSpacing = 0;
    printer.lineSpacing = 0;
    printer.x = 8;
    printer.currentX = 8;
...
void CreateYesNoMenu(const struct WindowTemplate *window, u16 baseTileNum, u8 paletteNum, u8 initialCursorPos)
{
    ...
    printer.currentX = printer.x;
    printer.currentY = printer.y;
    printer.color.foreground = 0x1; //GetFontAttribute(FONT_NORMAL, FONTATTR_COLOR_FOREGROUND);
    printer.color.background = 0x0; //GetFontAttribute(FONT_NORMAL, FONTATTR_COLOR_BACKGROUND); //=6
    printer.color.shadow = 0; //GetFontAttribute(FONT_NORMAL, FONTATTR_COLOR_SHADOW); //=7
    printer.color.accent = 0;//GetFontAttribute(FONT_NORMAL, FONTATTR_COLOR_ACCENT);
    printer.letterSpacing = 0;
    printer.lineSpacing = 0;
...
void PrintPlayerNameOnWindow(u8 windowId, const u8 *src, u16 x, u16 y)
{
    ...
    //AddTextPrinterParameterizedCustom(windowId, FONT_NORMAL, gStringVar4, x, y, TEXT_SKIP_DRAW, 0, 0x2, 0x0, 0x0);
}
...
void BufferSaveMenuText(u8 textId, u8 *dest, u8 color)
{
    ...
    *(string++) = 0x0; //was color + 1 //somehow this sets text shadow color, using standard message box's palette
    ...
    switch (textId)
    {
        ...
        case SAVE_MENU_BADGES:
            ...
            *endOfString = EOS;*/
           string = ConvertIntToDecimalStringN(string, VarGet(VAR_CURRENT_LEVEL_CAP), STR_CONV_MODE_LEFT_ALIGN, 2);
           *string = EOS;
            break;
    }
}
