...
enum DifficultyLevel GetTrainerDifficultyLevelTest(u16 trainerId)
{
    ...
    if (difficulty == DIFFICULTY_STORY)
...
TEST("Difficulty changes which party if used for NPCs if defined for the difficulty (CASUAL)")
{
    SetCurrentDifficultyLevel(DIFFICULTY_CASUAL);
    struct Pokemon *testParty = Alloc(6 * sizeof(struct Pokemon));
    u32 currTrainer = 2;
...
TEST("Difficulty changes which party if used for NPCs if defined for the difficulty (MERCILESS)")
{
...
TEST("Difficulty changes which party if used for NPCs if defined for the difficulty (STORY)")
{
    SetCurrentDifficultyLevel(DIFFICULTY_STORY);
    struct Pokemon *testParty = Alloc(6 * sizeof(struct Pokemon));
    u32 currTrainer = 2;
