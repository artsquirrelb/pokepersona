        ...
        if (!gPaletteFade.active)
        {
            ...
            else
            {
                if (BW_SUMMARY_SCREEN)
                    ...
                    ShowPokemonSummaryScreen_SwSh(SUMMARY_MODE_BOX_SELECT_MOVE, GetBoxedMonPtr(gSpecialVar_MonBoxId, 0), gSpecialVar_MonBoxPos, IN_BOX_COUNT - 1, CB2_InitLearnMoveReturnFromSelectMove);
                else
...
static void HideHeartSpritesAndShowTeachMoveText(bool8 onlyHideSprites)
{
    ...
    if (!onlyHideSprites)
    {
        ...
        FillWindowPixelBuffer(RELEARNERWIN_MSG, PIXEL_FILL(3));
        AddTextPrinterParameterizedCustom(RELEARNERWIN_MSG, FONT_NORMAL, gStringVar4, 0, 1, 0, NULL, 1, 0, 0);
    }
}
