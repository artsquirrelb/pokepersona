...
#define PSS_LABEL_WINDOW_PROMPT_RELEARN 9
...
static const u32 sSummaryMoveSelect_Gfx[]           = INCBIN_U32("graphics/summary_screen/swsh/move_select.4bpp.smol");
...
static const u32 sRelearnPromptMulti_Gfx[]          = INCBIN_U32("graphics/summary_screen/swsh/relearn_prompt_multi.4bpp.smol");
...
#define TAG_RELEARN_PROMPT_MULTI 30017
...
static const struct OamData sOamData_RelearnPromptMulti =
{
    .size = SPRITE_SIZE(64x32),
    .shape = SPRITE_SHAPE(64x32),
    .priority = 0,
...
static const struct CompressedSpriteSheet sSpriteSheet_RelearnPromptMulti =
{
    .data = sRelearnPromptMulti_Gfx,
    .size = 64*32*4/2,
    .tag = TAG_RELEARN_PROMPT_MULTI,
};
...
static const union AnimCmd *const sSpriteAnimTable_RelearnPromptMulti[] =
{
...
static const struct SpriteTemplate sSpriteTemplate_RelearnPromptMulti =
{
    .tileTag = TAG_RELEARN_PROMPT_MULTI,
    .paletteTag = TAG_MON_MARKINGS,
    .oam = &sOamData_RelearnPromptMulti,
    .anims = sSpriteAnimTable_RelearnPromptMulti,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCallbackDummy
};
...
void ShowSelectMoveBoxPokemonSummaryScreen_SwSh(struct BoxPokemon *mons, u8 monIndex, u8 maxMonIndex, void (*callback)(void), u16 newMove)
{
    ShowPokemonSummaryScreen_SwSh(SUMMARY_MODE_BOX_SELECT_MOVE, mons, monIndex, maxMonIndex, callback);
    sMonSummaryScreen->newMove = newMove;
}
...
static bool8 LoadGraphics(void)
{
    ...
    case 15:
        ResetSpriteIds();
        CreateMoveTypeIcons();
        if ((sMonSummaryScreen->mode == SUMMARY_MODE_SELECT_MOVE || sMonSummaryScreen->mode == SUMMARY_MODE_BOX_SELECT_MOVE) && SWSH_SUMMARY_CATEGORY_ICONS)
        ...
        if (P_SUMMARY_SCREEN_MOVE_RELEARNER)
            {
                ...
                LoadCompressedSpriteSheet(&sSpriteSheet_RelearnPromptMulti);
            }
        sMonSummaryScreen->switchCounter++;
        break;
    case 23:
...
bool32 CheckRelearnerStateFlag_SWSH(enum MoveRelearnerStates state)
{
    ...
    if (!sMonSummaryScreen->lockMonFlag)
    {
        ...
        if (monId != -1)
        {
            ...
            if (!gMain.inBattle)
            {
                ...
                if (isNewMonEgg || sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES)
...
static void PrintNewMoveDetailsOrCancelText(void)
{
    ...
    else
    {
        ...
        if (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES)
            PrintTextOnWindowToFitPx_WithFont(windowId1, GetMoveName(move), 4, moveIndex * 18 + 4, 0, 13, FONT_SMALL, WindowWidthPx(windowId1) - 3);
        else
            PrintTextOnWindowToFitPx_WithFont(windowId1, GetMoveName(move), 4, moveIndex * 18 + 4, 0, 13, FONT_SMALL, WindowWidthPx(windowId1) - 3);
        ...
        DynamicPlaceholderTextUtil_ExpandPlaceholders(gStringVar4, sMovesPPLayout);
        PrintTextOnWindowWithFont(windowId1, gStringVar4, 121, moveIndex * 18 + 4, 0, 4, FONT_SMALL);
    }
}
...
static void SetPokerusCuredSprite(void)
{
    ...
    gSprites[sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_POKERUS_CURED]].invisible = (IsPokerusInParty() || !CheckMonHasHadPokerus(mon));
}
...
static void ShowMoveRelearner(void)
{
    ...
    switch (gMoveRelearnerState)
    {
        case MOVE_RELEARNER_EGG_MOVES:
            ...
            break;
        case MOVE_RELEARNER_TM_MOVES:
            StartSpriteAnim(&gSprites[sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_RELEARN_PROMPT_MULTI]], MOVE_RELEARNER_TM_MOVES);
            break;
        case MOVE_RELEARNER_TUTOR_MOVES:
...
static void CB2_ReturnToSummaryScreenFromNamingScreenBox(void)
{
    ...
    ShowPokemonSummaryScreen_SwSh(SUMMARY_MODE_BOX, GetBoxedMonPtr(gSpecialVar_MonBoxId, 0), gSpecialVar_MonBoxPos, IN_BOX_COUNT - 1, gInitialSummaryScreenCallback);
}
