...
static void RemoveItemToGiveFromBag(enum Item);
...
static void DisplayLevelUpStatsPg1(u8 taskId)
{
    ...
    DrawLevelUpWindowPg1(arrayPtr[12], arrayPtr, &arrayPtr[6], 3, 1, 3);
...
static void DisplayLevelUpStatsPg2(u8 taskId)
{
    ...
    DrawLevelUpWindowPg2(arrayPtr[12], &arrayPtr[6], 3, 1, 3);
    CopyWindowToVram(arrayPtr[12], COPYWIN_GFX);
    ScheduleBgCopyTilemapToVram(2);
}
...
static u8 CheckBattleEntriesAndGetMessage(void)
{
    ...
    for (i = 0; i < maxBattlers - 1; i++)
    {
        // Skip unselected slots //(selectmons allows selecting fewer than max)<-actually no
        if (order[i] == 0)
            continue;
