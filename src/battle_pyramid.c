...
{
    {TRAINER_CLASS_KIRIJO_GENESIS_STAFF, TRAINER_ENCOUNTER_MUSIC_AQUA},
...
static void ShowPostBattleHintText(void)
{
    ...
    while (!i)
    {
        ...
        case HINT_REMAINING_ITEMS:
            ...
            {
                if (events[i].graphicsId == OBJ_EVENT_GFX_POKE_CHEST && events[i].x != SHRT_MAX && events[i].y != SHRT_MAX)
                    textIndex++;
            }
            i = 1;
            break;
...
static bool8 TrySetPyramidObjectEventPositionAtCoords(u8 objType, u8 x, u8 y, u8 *floorLayoutOffsets, u8 squareId, u8 objectEventId)
{
    ...
    for (i = 0; i < mapHeader->events->objectEventCount; i++)
    {
        ...
        if (objType != OBJ_TRAINERS || mapHeader->events->objectEvents[i].graphicsId == OBJ_EVENT_GFX_POKE_CHEST)
        {
            if (objType != OBJ_ITEMS || mapHeader->events->objectEvents[i].graphicsId != OBJ_EVENT_GFX_POKE_CHEST)
                continue;
        }
        ...
        if (j == objectEventId)
        {
            ...
            if (floorEvents[objectEventId].graphicsId != OBJ_EVENT_GFX_POKE_CHEST)
            {
