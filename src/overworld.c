...
/*static bool16 ShouldLegendaryMusicPlayAtLocation(struct WarpData *warp)
{
    ...
    if (warp->mapGroup == 0)
    {
        ...
        case MAP_NUM(MAP_ROUTE128):
            return TRUE;
        default:
            if (VarGet(VAR_CHAPTER_5_STATE) < 4)
                return FALSE;
            switch (warp->mapNum)
            {
...
{
    if (VarGet(VAR_CHAPTER_4_STATE) == 0)
        return FALSE;
...
const struct BlendSettings gTimeOfDayBlend[] =
    {
        [TIME_MORNING] = {.coeff = 4, .blendColor = 0xCF9D74, .isTint = TRUE}, // 0xA8B0E0 somehow morning and evening swapped in game
        [TIME_DAY] = {.coeff = 0, .blendColor = 0, .isTint = FALSE},
        [TIME_EVENING] = {.coeff = 6, .blendColor = 0xA8B0E0, .isTint = TRUE}, // 0xCF9D74
...
{
    s32 hours, minutes;
    RtcCalcLocalTime(); // already accounts for fake rtc
...
static void OverworldBasic(void)
{
    ...
    if (!gPaletteFade.active && --gTimeUpdateCounter <= 0)
    {
        ...
        if (MapHasNaturalLight(gMapHeader.mapType) &&
            (bld0[0] != bld1[0] || bld0[1] != bld1[1] || bld0[2] != bld1[2]))
        {
...
void CB2_Overworld(void)
{
    ...
    for (loops = 0; loops < OverworldSpeedup_AdditionalIterations(gSaveBlock2Ptr->optionsOverworldSpeed, TRUE); loops++)
    {
        AnimateSprites();
        CameraUpdate();
        UpdateCameraPanning();
...
static void CreateLinkPlayerSprite(u8 linkPlayerId, enum GameVersion gameVersion)
{
    ...
    if (linkPlayerObjEvent->active)
    {
        ...
        {
            u16 gfxId = GetLinkPlayerAvatarGraphicsIdByStateIdLinkIdAndGender(PLAYER_AVATAR_STATE_NORMAL, linkPlayerId);
...
{
    if ((gSaveBlock2Ptr->optionsOverworldSpeed != 0 && (JOY_HELD(R_BUTTON))) 
    || FlagGet(FLAG_PREVENT_OVERWORLD_SPEEDUP) 
    || FlagGet(FLAG_DN_SEARCHING))
    {
