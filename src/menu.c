...
static void MoveMenuGridCursor(u8 oldCursorPos, u8 newCursorPos)
{
    ...
    AddTextPrinterParameterized6(sMenu.windowId, sMenu.fontId, gText_SelectorArrow3, xPos, yPos, 0, 0, 0x2, 0x0, 0x0);
}
...
void PrintMenuTable(u8 windowId, u8 itemCount, const struct MenuAction *menuActions)
{
    ...
    for (i = 0; i < itemCount; i++)
        ...
        AddTextPrinterParameterized6(windowId, FONT_NORMAL, menuActions[i].text, 8, (i * 16) + 1, TEXT_SKIP_DRAW, NULL, 0x1, 0x0, 0x0);
    CopyWindowToVram(windowId, COPYWIN_GFX);
}
...
void AddTextPrinterParameterized6New(u8 windowId, u8 fontId, u8 left, u8 top, u8 letterSpacing, u8 lineSpacing, const union TextColor color, s8 speed, const u8 *str)
{
    struct TextPrinterTemplate printer;
